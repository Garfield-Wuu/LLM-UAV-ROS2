...
namespace ego_planner
{
    ...
    void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
    {
        ...
        if (success)
        {
            ...
            {
                // In ROS2 callback context, nested spin_some(node_) can throw
                // "Node has already been added to an executor". Use non-blocking
                // state transition and let the FSM timer drive progression.
    ...
    void EGOReplanFSM::waypointCallback(const std::shared_ptr<const geometry_msgs::msg::PoseStamped>& msg)
    {
        if (!have_odom_)
        {
            RCLCPP_WARN(node_->get_logger(), "Ignore goal: odom not ready yet.");
            return;
        }
