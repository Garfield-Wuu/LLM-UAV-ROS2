...
> **最后更新**：2026-03-27（第六次：技术路线确定 —— YOLO-World + VINS-Fusion + EGO-Planner 完整闭环架构文档同步）  
> **关联 PRD**：`PRD_text_command_flight_mvp.md`（v4.2）  
> **历史会话 ID**：
...
> - 第六次：当前（技术路线锁定：YOLO-World 视觉识别 + AirSim 深度 Grounding + VINS-Fusion 位姿对齐 + EGO-Planner 局部规划；文档同步）
...
| 视觉语义识别 | **YOLO-World**（open-vocabulary 目标检测，`prompt-then-detect` 范式）| 📋 规划中 |
...
- 新建 `vision_grounding_node.py`（或 C++ 节点），集成 YOLO-World 推理
...
├── COMMAND_PROTOCOL.md
├── PRD_text_command_flight_mvp.md ← v4.2
