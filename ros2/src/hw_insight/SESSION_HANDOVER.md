...
用户自然语言指令
  ...
  ▼
llm_client.py（任务理解）
  ├── 当前已实现：输出飞行动作 JSON 到 /uav/user_command
  └── 目标升级：输出结构化视觉任务描述，并生成 /uav/target_query
  │
  ▼
yolo_world_detector.py + target_grounding_node.py + semantic_target_tf_node.py
  ├── 订阅 /airsim_node/PX4/CameraDepth1/Scene（RGB）
...
| `hw_insight/yolo_world_detector.py` | S9 | YOLO-World ROS 2 检测节点；订阅 RGB + prompt → `/uav/detections_2d` JSON；GPU 推理；线程化降频 |
| `hw_insight/target_grounding_node.py` | S9 | 深度 grounding 节点；bbox 区域中位数深度 + 相机内参逆投影 → `/uav/semantic_targets_camera` |
| `hw_insight/semantic_target_tf_node.py` | S9 | camera→body→world 坐标变换节点；AirSim odom NED→ENU；RViz Marker 可视化；可选发布 `/uav/target_goal` |
| `hw_insight/semantic_goal_to_planner.py` | S9 | 薄桥接节点；从 `/uav/semantic_targets_world` 选最高分目标 → `/uav/target_goal`（PoseStamped） |
| `launch/semantic_perception.launch.py` | S9 | 语义感知链独立 launch（detector + grounding + tf_node，参数化 prompt/topic/odom） |
...
| `hw_insight/llm_client.py` | S11 | **① FIND_AND_GOTO** 新增到 `ALLOWED_ACTIONS`；**② 8 个别名**（FIND / SEARCH / LOCATE 等）加入 `_ACTION_ALIASES`；**③** `_PARAM_SCHEMA` 新增 `('query', str, None, None, True)`；**④** `_SYSTEM_PROMPT_TEMPLATE` 新增视觉搜索示例；**⑤** `_parse_and_validate_single` 跳过 altitude/speed 截断；**⑥** query 为空时回落 HOVER |
| `hw_insight/text_command_bridge.py` | S11 | **① 新增发布者** `/uav/target_query`；**② 新增订阅者** `/uav/semantic_targets_world`；**③ 搜索状态机**（`_search_active / _search_query / _search_deadline_ns`，超时 20s）；**④ FIND_AND_GOTO 分支**（验证 query → 发布 prompt → 进入 SEARCHING 状态）；**⑤ `_cancel_search`** 辅助方法（超时 HOVER + SEARCH_FAILED 事件）；**⑥ `_on_semantic_targets_world`**（过滤置信度 ≥ 0.15 → ENU→NED 转换 → 内部 GOTO_NED）；**⑦ TELEMETRY** 新增 `searching_query` / `search_remaining_sec` 字段 |
| `COMMAND_PROTOCOL.md` | S11 | v2.1→v2.2；新增第 12 个 action FIND_AND_GOTO；更新 Current Scope（Gap 已消除）；新增 SEARCH_FOUND/SEARCH_FAILED 事件；TELEMETRY 新增搜索字段；Coordinate System Notes 第 5 条 ENU→NED 说明 |
...
### 3.2 当前语义感知链（`semantic_perception.launch.py`）
...
yolo_world_detector.py
    ...
    └── /uav/detections_2d（JSON：label/score/bbox/stamp/frame_id）
    ▼
target_grounding_node.py
    ├── bbox 区域中位数深度
    ├── camera_info 逆投影
    └── /uav/semantic_targets_camera（JSON：u/v/depth/x_cam/y_cam/z_cam）
    ▼
semantic_target_tf_node.py
    ├── camera_optical → body → world
    ├── NED → ENU
    ├── /uav/semantic_targets_world
    ├── /uav/semantic_target_marker
    └── 可选 /uav/target_goal

semantic_goal_to_planner.py（可选）
    └── 从 /uav/semantic_targets_world 选主目标 → /uav/target_goal
```
...
> **S8 完成**：本地部署与推理验证。  
> **S9 完成**：ROS 2 实时检测节点 `yolo_world_detector.py`。

- ✅ YOLO-World-S 模型部署（权重 + CLIP 离线化，GPU 优先）
- ✅ 开放词汇推理验证（bus.jpg / zidane.jpg）
- ✅ `yolo_world_detector.py`：订阅 RGB + prompt → `/uav/detections_2d`（JSON：label/score/bbox/stamp/frame_id）
- ✅ 支持静态参数 `texts` 与动态 `/uav/target_query` 双 prompt 来源
- ✅ GPU 推理 + 线程化 + 降频保护（`min_inference_interval_sec`）
...
- ✅ 固定深度类型 DepthPlanar；bbox 区域中位数深度；过滤 0/NaN/越界
- ✅ 基于 `camera_info` 的 `fx/fy/cx/cy` 逆投影 → camera frame 3D 点
- ✅ 输出 `/uav/semantic_targets_camera`（JSON：u/v/depth_m/x_cam/y_cam/z_cam）
- ✅ 本轮联调前提：AirSim 相机已前移，无人机在约 6 米高度测试
- 🔲 验收：已知距离目标，camera frame 坐标估计误差 <20%

#### P5-C：坐标系变换（camera→body→world）

> **S9 完成**：`semantic_target_tf_node.py`。

- ✅ camera_optical→body NED 固定外参（可配 RPY 修正 + 平移偏移）
...
> **S9 完成**：`semantic_goal_to_planner.py`（薄桥接）。

- ✅ 订阅 `/uav/semantic_targets_world` → 选最高分 → `/uav/target_goal`
- ✅ `semantic_target_tf_node` 自身也可直接发 `/uav/target_goal`（二选一）
- 🔲 `mission_manager`：LLM 速度模式 vs planner 模式互斥与抢占策略
- 🔲 安全层：规划速度上限、地理围栏、超时策略联调
- 🔲 验收：输入 `"找到目标并前往"` 后，无人机能在 AirSim 中自主定位并接近目标
...
- 🔲 设计“飞到目标上方/附近/前方”等相对目标语义，映射为 planner 目标点策略
- 🔲 明确与现有 `MOVE_REL` / `GOTO_NED` / planner 模式的互斥与优先级
- 🔲 验收：`“飞到穿黄色衣服人的头顶”` 能正确拆解为“识别目标 + 生成 world 点 + 发布目标点 + 执行规划”
...
│   ├── yolo_world_detector.py    ← YOLO-World ROS 2 检测节点（GPU 推理，降频保护）
│   ├── target_grounding_node.py  ← 2D bbox + 深度中位数 → camera frame 3D 点
...
│   ├── semantic_perception.launch.py ← 语义感知链（detector+grounding+tf_node）
...
| 语义感知消息格式 | `std_msgs/String`（JSON） | 自定义 msg | 先让数据流稳定再迁移到 `hw_interface` 自定义消息；避免早期接口频繁改动 |
| 检测→grounding→TF 三层拆分 | 每层独立节点 | 单一大节点 | 逐层可独立验收和调试；话题解耦便于替换实现 |
| camera→body 外参 | 节点参数 `cam_to_body_rpy_deg` + 默认光学→NED 旋转 | tf2 static_transform_publisher | 避免引入额外进程；参数化修正足够灵活 |
| NED→ENU 转换 | `semantic_target_tf_node` 内 `odom_frame_is_ned` 参数控制 | 外部 remap | 集中处理避免多处散落 |
| 推理降频 | `min_inference_interval_sec` + 线程化 | 固定频率订阅 | GPU 推理速度不确定，按最新帧降频比固定频率更灵活 |
| YOLO-World 图像输入 | 临时文件写盘 → mmdet pipeline 读取 | 内存 ndarray 直接传入 | mmdet pipeline 设计为读文件；避免改上游代码；/tmp 写入开销在 GPU 推理面前可忽略 |
...
14. **深度与 RGB 时间戳对齐**：`target_grounding_node` 用 `max_depth_age_sec` 参数（默认 0.5s）拒绝过旧深度帧。如果 AirSim 深度和 RGB 发布频率差异大，可能需要调大此参数或确认两个相机发布频率一致。

15. **`semantic_target_tf_node` 与 `semantic_goal_to_planner` 二选一发 `/uav/target_goal`**：两个节点默认都会发，可能产生竞争。建议初期只用 `semantic_target_tf_node` 的 `publish_target_goal:=true`，不启动 `semantic_goal_to_planner`，待需要更复杂的目标选择策略时再切换。

16. **YOLO-World 并非只能 CPU 推理**：代码路径是 GPU 优先，且历史上已单独验证过 GPU 推理成功。若当前会话里出现 CPU fallback，通常是 Python 环境的 `torch` CUDA build 与驱动版本不匹配，而不是节点设计问题。

17. ~~**当前本机 Python 环境存在 CUDA 版本错配**~~：**已修复（S11）**。原因为 `torch 2.11.0+cu130` 与驱动 CUDA 12.9 不兼容；已通过 `pip install --force-reinstall torch torchvision --index-url https://download.pytorch.org/whl/cu128` 解决，当前 `torch.cuda.is_available() = True`。

18. ~~**自然语言视觉任务尚未真正接入 LLM 动作协议**~~：**已实现（S11）**。`llm_client.py` 新增 `FIND_AND_GOTO` action 及 8 个别名；`text_command_bridge.py` 实现完整视觉搜索状态机；LLM 可解析"飞到穿黄色衣服人的头顶"并自动驱动 YOLO-World 检测 → ENU→NED 转换 → GOTO_NED 飞行。详见 COMMAND_PROTOCOL.md §12。
